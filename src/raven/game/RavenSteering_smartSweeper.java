...
//import raven.game.RavenSteering.BehaviorType;
//import raven.game.RavenSteering.Deceleration;




import raven.game.interfaces.IRavenBot;
import raven.game.interfaces.IRavenSteering;
...
import smartSweeper.*;
import smartSweeper.jGenAlg.SGenome;

public class RavenSteering_smartSweeper implements IRavenSteering {
	...
	//-------------------------End of NN Stuff-----------------------------//
		...
		/**function to check feelers for collision*/
		
		//////////////Check Feelers method is here///////////////////////////////
		private Vector<Double> checkFeelers(List<Wall2D> walls){
			Vector<Double> bumps = new Vector<Double>();
			createFeelers();
			...
			for (int flr=0; flr<feelers.size(); ++flr)
			{
				...
				if (ClosestWall >=0)
				{
					if (DistToThisIP < feelers.get(flr).length() + 1000.0){
						bumps.add(1.0);
						currentGenome.dFitness -= 3.0;
						System.out.println("BUMP!");
					}
						
					else{
						bumps.add(-1.0);
						currentGenome.dFitness += 0.01;
					}
			...
			if(currentGenome.dFitness < 0.0) currentGenome.dFitness = 0.0;
			if(currentGenome.dFitness > 100.0) currentGenome.dFitness = 100.0;
			return bumps;
		}
...
//			if (DistToClosestIP > 1000.0) fitness += 0.1;
//			else fitness -= 0.1;
		...
		private Vector2D calculatePrioritized(){
			...
			Vector<Double> inputs = new Vector<Double>(3);
			Vector<Double> outputs = new Vector<Double>(2);
			createFeelers();
			Double DistToThisIP    = 0.0;
			double DistToClosestIP = Double.MAX_VALUE;
			List<Wall2D> walls = world.getMap().getWalls();

			
//			System.out.print("Steering calling update. Input size: ");
//			System.out.println(inputs.size());
			outputs = brain.Update(checkFeelers(walls));
			try{
				force.x = outputs.firstElement();
				force.y = outputs.lastElement();
				Double oldMag = force.length();
				force.x -= oldMag/2;
				force.y -= oldMag/2;
				accumulateForce(steeringForce, force);
			}catch (java.util.NoSuchElementException e){
				System.out.print("Wrong number of outputs. Number found: ");
				System.out.println(outputs.size());
			}
//			System.out.print("Magnitude of output from brain: ");
//			System.out.println(force.length());
			System.out.print("Force in x direction: ");
			System.out.println(force.x);
			System.out.print("Force in y direction: ");
			...
			accumulateForce(steeringForce, force);
			if (ticks % 100 == 0) updateBrain();
			ticks++;
			return steeringForce;
		}


		public RavenSteering_smartSweeper(RavenGame world, RavenBot ravenBot) {
		...
		private void updateBrain(){
			population = ga.Epoch(population);
			currentGenome = population.firstElement();
			if(currentGenome.vecWeights != null && currentGenome.vecWeights.size() == 15)
				brain.PutWeights(currentGenome.vecWeights);
			else
				System.out.println("currentGenome.vecWeights has wrong size");
...
//				System.out.print(" ");
//			}
			//System.out.println("Current fitness: " + currentGenome.dFitness);
			System.out.println("Current tick: " + ticks++);
			//brain.PutWeights(newPop.firstElement().vecWeights);
			//System.out.print("Current fitness: ");
			//System.out.println(fitness);
		}
		
		private class TrainingCase{
			public Vector<Double> expectedInput;
			public Vector<Double> expectedOutput;
			
			public TrainingCase(Vector<Double> input, Vector<Double> output){
				expectedInput = input;
				expectedOutput = output;
			}
		}
		
		private double computeFarness
			(Vector<Double> expectedOutput, Vector<Double> generatedOutput){
			double retval = 0.0;
			
			for (int i = 0; i < expectedOutput.size(); ++i){
				retval += Math.abs(expectedOutput.get(i) - generatedOutput.get(i));
			}
		...
		private void trainBrain(){
			int initialPopSize = 10;
			
			//Build 4 training cases:
			//{0,0,0}, {1,0,0}, {0,1,0}, {0,0,1} inputs
			Vector<TrainingCase> trainingCases = new Vector<TrainingCase>();
			Vector<Double> input = new Vector<Double>();
			Vector<Double> output = new Vector<Double>();
			input.add(0.0);input.add(0.0);input.add(0.0);
			output.add(1.0); output.add(1.0);
			trainingCases.add(new TrainingCase(input, output));
			
			input.clear(); output.clear();
			input.add(1.0);input.add(0.0);input.add(0.0);
			output.add(1.0); output.add(0.0);
			trainingCases.add(new TrainingCase(input, output));
			
			input.clear(); output.clear();
			input.add(0.0);input.add(1.0);input.add(0.0);
			output.add(-1.0); output.add(1.0);
			trainingCases.add(new TrainingCase(input, output));
			
			input.clear(); output.clear();
			input.add(0.0);input.add(0.0);input.add(1.0);
			output.add(0.0); output.add(1.0);
			trainingCases.add(new TrainingCase(input, output));
		
			Vector<jGenAlg.SGenome> trainingPop = new Vector<jGenAlg.SGenome>();
			for (int i = 0; i < initialPopSize; i++){
				trainingPop.add(ga.new SGenome());
			}
		}
