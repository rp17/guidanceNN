...
public class RavenSteering_gANN implements IRavenSteering{
	
	//----------------------------Neural Nodes------------------
	
	GNeuron output, hidden1, hidden2, hidden3;
	GFeelerNode in1, in2, in3;
	...
	private Vector2D calculatePrioritized(){
		...
		in1.setFeeler(feelers.get(0));
		in2.setFeeler(feelers.get(0));
		in3.setFeeler(feelers.get(0));
		
		in1.setWalls(world.getMap().getWalls());
		in2.setWalls(world.getMap().getWalls());
		in3.setWalls(world.getMap().getWalls());
		
		Vector2D oldForce = ravenBot.heading;
		if (toStart){
			System.out.println("Starting bot");
			toStart = false;
			steeringForce.x = 0.1f;
			steeringForce.y = 0.1f;
		}
		else{
			//steeringForce.Zero();
			if(output.getOutput() < 05.){
				//System.out.println("Turning right");
				force.x = 0.707*oldForce.x - 0.707*oldForce.y;
				force.y = 0.707*oldForce.x + 0.707*oldForce.y;
			}
			else{
		...
		//output.backProp(0.0f, hasHit.value?1.0f:0.0f);
		
		return steeringForce;
	}


	public RavenSteering_gANN(RavenGame world, RavenBot ravenBot) {
		...
		System.out.print("initializing bot ");
		System.out.println(ravenBot.ID());
		
		output = new GNeuron();
		hidden1 = new GNeuron();
		hidden2 = new GNeuron();
		hidden3 = new GNeuron();
		in1 = new GFeelerNode(ravenBot);
		in2 = new GFeelerNode(ravenBot);
		in3 = new GFeelerNode(ravenBot);
		
		output.addInput(hidden1);
		output.addInput(hidden2);
		output.addInput(hidden3);
		
		hidden1.addInput(in1);
		hidden1.addInput(in2);
		hidden1.addInput(in3);
		
		hidden2.addInput(in1);
		hidden2.addInput(in2);
		hidden2.addInput(in3);
		
		hidden3.addInput(in1);
		hidden3.addInput(in2);
		hidden3.addInput(in3);
		
		in1.setHasHit(hasHit);
		in2.setHasHit(hasHit);
		in3.setHasHit(hasHit);
